...
export class ReminderRepository {
    ...
    async saveReminder(params) {
        ...
        const result = await this.pool.query(`INSERT INTO reminders (id, user_id, chat_id, text, schedule_type, schedule_value, start_date, end_date, timezone)
    ...
    async getReminders(userId) {
        const result = await this.pool.query(`SELECT * FROM reminders
    ...
    async getReminderForUser(reminderId, userId) {
        const result = await this.pool.query(`SELECT * FROM reminders
       WHERE id = $1 AND user_id = $2 AND active = TRUE`, [reminderId, userId]);
        return result.rows[0] ? this.mapDbRowToReminder(result.rows[0]) : null;
    }
    /**
    ...
    async setUserTimezone(userId, timezone) {
        await this.pool.query(`INSERT INTO users (id, timezone) VALUES ($1, $2)
       ON CONFLICT (id) DO UPDATE SET timezone = $2`, [userId, timezone]);
