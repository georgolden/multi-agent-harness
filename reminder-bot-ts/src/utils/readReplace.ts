...
export function replaceVars(content: string, variables: Record<string, any>): string {
  const varNames = Object.keys(variables);
  if (varNames.length === 0) return content;
...
export async function loadAndReplace(filePath: string, variables: Record<string, any>): Promise<string> {
  try {
